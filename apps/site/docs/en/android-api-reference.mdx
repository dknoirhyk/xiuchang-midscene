...
- `imeStrategy?: 'always-yadb' | 'yadb-for-non-ascii' | 'clipboard' | 'adb-keyboard'` &mdash; Choose when to invoke [yadb](https://github.com/ysbing/YADB) for text input. Default `'yadb-for-non-ascii'`.
  ...
  - `'clipboard'` &mdash; Writes text to the device clipboard via yadb (`-writeClipboard`), then sends `KEYCODE_PASTE`. Unlike direct keyboard injection, paste goes through Android's standard `InputConnection`, which allows apps to properly clear hint/placeholder text before inserting. Useful for input fields where direct injection causes text to be appended to existing hint content.
  - `'adb-keyboard'` &mdash; Switches the device IME to [ADBKeyboard](https://github.com/senzhk/ADBKeyBoard), sends text via broadcast (`ADB_INPUT_B64`), then restores the original IME. Like `'clipboard'`, this goes through `InputConnection` and correctly handles hint text clearing. Requires ADBKeyboard to be installed on the device.
