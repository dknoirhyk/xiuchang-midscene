...
- `imeStrategy?: 'always-yadb' | 'yadb-for-non-ascii' | 'clipboard' | 'adb-keyboard'` —— 控制何时调用 [yadb](https://github.com/ysbing/YADB) 进行文本输入，默认 `'yadb-for-non-ascii'`。
  ...
  - `'clipboard'` —— 通过 yadb 的 `-writeClipboard` 命令将文字写入设备剪贴板，然后发送 `KEYCODE_PASTE` 粘贴。与直接键盘注入不同，粘贴操作经过 Android 标准的 `InputConnection`，允许应用在插入内容前正确清除底纹词（hint/placeholder）。适用于直接注入会导致文字追加到底纹词后面的输入框。
  - `'adb-keyboard'` —— 将设备输入法切换为 [ADBKeyboard](https://github.com/senzhk/ADBKeyBoard)，通过广播（`ADB_INPUT_B64`）发送文字，输入完成后还原原输入法。与 `'clipboard'` 类似，该方式经过 `InputConnection`，可正确处理底纹词清除，且对所有 Unicode 字符无障碍。需要设备已安装 ADBKeyboard。
